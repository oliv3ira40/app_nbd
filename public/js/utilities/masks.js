$(function() {
    ...
    var mask_cpf_or_cnpj = $(".mask_cpf_or_cnpj");
    if (mask_cpf_or_cnpj.length > 0) {
        mask_cpf_or_cnpj.mask('00.000.000/0000-00');
    
        mask_cpf_or_cnpj.on('input', function() {
            var length_mask_cpf_or_cnpj = mask_cpf_or_cnpj.val().replace(/\D/g, '').length;
            
            if (length_mask_cpf_or_cnpj <= 11) {
                mask_cpf_or_cnpj.mask('000.000.000-00');
            } else {
                mask_cpf_or_cnpj.mask('00.000.000/0000-00');
            }
        });
